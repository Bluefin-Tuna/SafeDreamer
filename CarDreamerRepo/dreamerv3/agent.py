...
# from jax.scipy.signal import medfilt
        ...
        if mode not in ['train', 'eval', 'explore']:
            ...
            if 'surprise' in mode:
                ...
                def median_blur(image: jnp.ndarray, ksize: int) -> jnp.ndarray:
                    ...
                    padded = jnp.pad(image,
                                    ((0, 0), (pad, pad), (pad, pad), (0, 0)),
                                    mode="reflect")

                    # Extract patches using dynamic slicing
                    def get_patch(n, i, j):
                        return jax.lax.dynamic_slice(padded, (n, i, j, 0), (1, ksize, ksize, C)).reshape(-1, C)
                    ...
                    patches = jax.vmap(  # over batch
                        lambda n: jax.vmap(  # over H
                            lambda i_row, j_row: jax.vmap(get_patch, in_axes=(None, 0, 0))(n, i_row, j_row),
                            in_axes=(0, 0)
                        )(ii, jj),
                        in_axes=(0,)
                    )(jnp.arange(N))  # shape (N, H, W, ksize*ksize, C)
                ...
                n_samples = 70
                target_dropout_ratios = np.linspace(.05, 1, n_samples)
                
                total_newlat_seperate = []
                surprises = []
                key = random.PRNGKey(42)
                # done = False  # Python bool at start
                surprise_threshold = 120 #py_rand.gauss(10, 10)#10 +  # Example threshold, tune based on your domain
