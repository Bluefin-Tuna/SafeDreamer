...
def wrap_env(env, config):
    args = config.wrapper
    env = wrappers_v3.InfoWrapper(env)
    for name, space in env.act_space.items():
        if name == "reset":
            continue
        elif space.discrete:
            env = wrappers_v3.OneHotAction(env, name)
        elif args.discretize:
            env = wrappers_v3.wrappers.DiscretizeAction(env, name, args.discretize)
        else:
            env = wrappers_v3.wrappers.NormalizeAction(env, name)
    env = wrappers_v3.wrappers.ExpandScalars(env)
    if args.length:
        env = wrappers_v3.wrappers.TimeLimit(env, args.length, args.reset)
    if args.checks:
        env = wrappers_v3.wrappers.CheckSpaces(env)
    for name, space in env.act_space.items():
        if not space.discrete:
            env = wrappers_v3.wrappers.ClipAction(env, name)
    return env
