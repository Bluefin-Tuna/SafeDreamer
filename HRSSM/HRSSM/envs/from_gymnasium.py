...
class FromGymnasium(base.Env):
  ...
  def obs_space(self):
    ...
    return {
        **spaces,
        ...
        'cost': spacelib.Space(np.float32),
  ...
  def step(self, action):
    ...
    # print(action)
    if 'reset' in action.keys() or self._done:
    #if self._done:
      self._done = False
      obs, info = self._env.reset()
      # obs = obs[:] #print('Obs coming:',obs[:][0])
      return self._obs(obs, 0.0, 0.0, is_first=True)
    if self._act_dict:
    ...
    return final_obs#, final_obs['reward'], (final_obs['is_last'] or final_obs['is_terminal']), {}
