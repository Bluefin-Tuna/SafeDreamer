...
def wrap_env(env, config):
    ...
    for name, space in env.act_space.items():
        ...
        #     env = wrappers_v3.DiscretizeAction(env, name, args.discretize)
        else:
            env = wrappers_v3.NormalizeAction(env, name)
    # env = wrappers_v3.ExpandScalars(env)
    if args.length:
        env = wrappers_v3.TimeLimit(env, args.length, args.reset)
    if args.checks:
        env = wrappers_v3.CheckSpaces(env)
    for name, space in env.act_space.items():
        if not space.discrete:
            env = wrappers_v3.ClipAction(env, name)
    return env
