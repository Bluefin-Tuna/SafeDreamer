...
class WorldModel(nn.Module):
    def __init__(self, obs_space, act_space, step, config):
        ...
        shapes = {k: tuple(v.shape) for k, v in obs_space.items()} #Geigh
    ...
    def _train(self, data):
        ...
        data['birdeye_wpt'] = masked_image #Geigh Zollicoffer Image and bird eye are exactly the same.
    ...
    def preprocess(self, obs):
        ...
        if "birdeye_wpt" in obs:
            obs["birdeye_wpt"] = torch.Tensor(obs["birdeye_wpt"]) / 255.0 - 0.5
        if "discount" in obs:
